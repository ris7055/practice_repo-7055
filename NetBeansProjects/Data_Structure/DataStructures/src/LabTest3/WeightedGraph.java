         ...
         if (sourceVertex.vertexInfo.compareTo( source ) == 0 )   {
            ...
            while (destinationVertex!=null)	{
               if ( destinationVertex.vertexInfo.compareTo( destination ) == 0 )   {
                  ...
                  sourceVertex.firstEdge=newEdge;
                  /*sourceVertex.outdeg++;
                  destinationVertex.indeg++;*/  // The solution is to remove indeg and outdeg
                  return true;
               }
               destinationVertex=destinationVertex.nextVertex;
            }
   ...
   public Edge<T, N> removeEdge(T source, T destination) {
      ...
      while (sourceVertex!=null)	{
         if ( sourceVertex.vertexInfo.compareTo( source ) == 0 )   {
            ...
            Edge<T,N> currentEdge = sourceVertex.firstEdge;
            Edge<T,N> lastEdge = null;
            while (lastEdge != currentEdge) {
               while (currentEdge.nextEdge != lastEdge)
                   currentEdge = currentEdge.nextEdge;
               if (lastEdge.toVertex.vertexInfo.compareTo(destination)==0) {
                   Edge<T, N> tempEdge = sourceVertex.firstEdge;
                   while (tempEdge.nextEdge != lastEdge) {
                       tempEdge = tempEdge.nextEdge;
                   }
                   tempEdge.nextEdge = tempEdge.nextEdge.nextEdge;
               }
               lastEdge = currentEdge;
               currentEdge = sourceVertex.firstEdge; 
            }
         }
         sourceVertex=sourceVertex.nextVertex;
      }
